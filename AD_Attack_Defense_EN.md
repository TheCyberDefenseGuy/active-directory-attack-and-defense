...
```mermaid
mindmap
  root((T1134 Access Token Manipulation))
    T1134.001 Token Impersonation and Theft
    ...
    T1134.002 Create Process with Token
      ...
      Spawn shell under stolen identity
    T1134.003 Make and Impersonate Token
      LogonUser API + ImpersonateLoggedOnUser
      Requires cleartext credentials
    T1134.004 Parent PID Spoofing
      Hide malicious process under legitimate parent
      Evades parent-based detection
    T1134.005 SID-History Injection
      Add foreign domain SID to account SID-History
...
```mermaid
mindmap
  root((AD Persistence Strategies))
    Domain-Level
      Golden Ticket
        KRBTGT hash - T1558.001
      DCSync Backdoor
        Grant replication rights to low-priv account
      AdminSDHolder ACL Injection
        Survives SDProp cycle
      Malicious GPO
        Startup script or scheduled task
    Host-Level
      Scheduled Task T1053.005
      Registry Run Keys T1547.001
      Malicious Service T1543.003
