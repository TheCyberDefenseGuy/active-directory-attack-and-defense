...
mindmap
 root((T1134\nManipulação de\nAccess Token))
 T1134.001[" Roubo e\nImpersonation de Token\nRouba token de processo privilegiado\nRequer SeDebugPrivilege\nou SeImpersonatePrivilege"]
 T1134.002[" Criar Processo\ncom Token\nCreateProcessWithTokenW\nCreateProcessAsUserA\nShell sob identidade roubada"]
 T1134.003[" Criar e Impersonar\nToken\nAPI LogonUser\nImpersonateLoggedOnUser\nPrecisa de credenciais em texto claro"]
 T1134.004[" Spoofing de\nPID Pai\nOculta processo malicioso\nsob pai legítimo\nEngana detecção baseada em pai"]
 T1134.005[" Injeção de\nSID-History\nAdiciona SID de domínio externo\nao SID-History da conta\nAbuso de privilégio entre domínios"]
```
...
mindmap
 root((Estratégias de\nPersistência em AD))
 Domain[" Nível de Domínio"]
 GoldenTkt["Golden Ticket\nHash KRBTGT\nT1558.001"]
 DCSync["Backdoor DCSync\nConcedem direitos de replicação\na conta de baixo privilégio"]
 AdminSDH["AdminSDHolder\nInjeção de ACL\nSobrevive ao ciclo SDProp"]
 MalGPO["GPO Maliciosa\nScript de startup\nou tarefa agendada"]
 Host[" Nível de Host"]
 Sched["Tarefa Agendada\nT1053.005"]
 RegRun["Chaves Run do Registro\nT1547.001"]
 Service["Serviço Malicioso\nT1543.003"]
 OutlookHP["Outlook Home Page\nT1137.004"]
 Credential[" Baseado em Credencial"]
 SIDHist["Injeção de SID-History\nT1134.005\nAcesso entre domínios"]
 ShadowCred["Shadow Credentials\nmsDS-KeyCredentialLink"]
 ADCS["Abuso de Template ADCS\nCaminhos ESC1-8"]
```
